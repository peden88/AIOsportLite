...
async function tmdbBundle(type,tmdbId){
  ...
  const media=type==='movie'?'movie':'tv',headers=token?{Authorization:'Bearer '+token}:{},qs=(apiKey?'api_key='+encodeURIComponent(apiKey)+'&':'')+'append_to_response=credits,videos,recommendations,similar,external_ids,release_dates,content_ratings&language=en-US';
...
async function tvdbToken(){
  ...
  try{const body=await publicJson('https://api4.thetvdb.com/v4/login',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(pin?{apikey:apiKey,pin}:{apikey:apiKey})});const token=String(body?.data?.token||body?.token||'');return token?putCache('tvdb:token',token,20*60*60*1000):''}catch(_){return ''}
}
async function tvdbFallback(type,ids){
  const token=await tvdbToken();if(!token)return null;const headers={Authorization:'Bearer '+token},key='tvdb:'+type+':'+String(ids.imdb||ids.tmdb||'');const c=cached(key,24*60*60*1000);if(c)return c;
  try{
    let search=null;if(ids.imdb){const q=await publicJson('https://api4.thetvdb.com/v4/search?query='+encodeURIComponent(ids.imdb)+'&type='+encodeURIComponent(type==='movie'?'movie':'series'),{headers});search=q?.data?.[0]||null}
    if(!search)return null;const id=search.tvdb_id||search.id;if(!id)return null;
    const path=type==='movie'?'movies':'series',body=await publicJson('https://api4.thetvdb.com/v4/'+path+'/'+encodeURIComponent(id)+'/extended?meta=translations',{headers});
    return putCache(key,body?.data||body,24*60*60*1000);
  }catch(_){return null}
}
function tvdbPeople(body){
  const out=[];for(const row of (Array.isArray(body?.characters)?body.characters:[])){const p=Array.isArray(row?.people)?row.people[0]:(row?.person||row?.people||{});const name=p?.name||row?.personName||row?.person_name||row?.name;if(!name)continue;out.push({name:String(name),character:String(row?.name||row?.role||row?.type||'Cast'),photo:String(row?.image||row?.image_url||p?.image||p?.image_url||'')})}return out;
}
function mergePeople(primary,fallback){
  const map=new Map();for(const row of [...(primary||[]),...(fallback||[])]){if(!row?.name)continue;const key=String(row.name).toLowerCase();const old=map.get(key);if(!old)map.set(key,{...row});else map.set(key,{...old,character:old.character&&old.character!=='Cast'?old.character:row.character||old.character,photo:old.photo||row.photo||'',tmdbId:old.tmdbId||row.tmdbId||null})}return [...map.values()];
}
async function mdbListMeta(imdbId){
  const apiKey=String(process.env.MDBLIST_API_KEY||process.env.MDBLIST_APIKEY||'').trim();if(!apiKey||!imdbId)return null;
  const key='mdblist:'+imdbId,c=cached(key,24*60*60*1000);if(c)return c;
  try{return putCache(key,await publicJson('https://api.mdblist.com/?apikey='+encodeURIComponent(apiKey)+'&i='+encodeURIComponent(imdbId),{timeoutMs:8000}))}catch(_){return null}
}
function mdbRatings(body){
  const out=[];if(!body||typeof body!=='object')return out;
  const add=(source,value)=>{const n=Number(value);if(source&&Number.isFinite(n)&&n>0&&!out.some(x=>x.source===source))out.push({source,value:n})};
...
function mergeTmdbRelated(tmdb,type){
  const rows=[...(tmdb?.recommendations?.results||[]),...(tmdb?.similar?.results||[])],seen=new Set(),out=[];
  for(const row of rows){const item=tmdbPreview(row,type),key=String(item.tmdbId||item.id);if(!item.id||!item.name||seen.has(key))continue;seen.add(key);out.push(item)}
  return out;
}
...
async function enrichment(type,id){
  const safeType=validStremioType(type),safeId=cleanId(id),baseBody=await meta(safeType,safeId),base=baseBody?.meta||{},ids=parseIds(base,safeId),cine=(await cinemetaMeta(safeType,ids.imdb))?.meta||{},merged={...cine,...base},mergedIds=parseIds(merged,safeId),tmdbId=await tmdbResolve(safeType,mergedIds),[tmdb,mdb,tvdb]=await Promise.all([tmdbBundle(safeType,tmdbId),mdbListMeta(mergedIds.imdb),tvdbFallback(safeType,mergedIds)]);
  const creators=[],cast=[];const tvdbCast=tvdbPeople(tvdb);
  ...
  const ratings=mdbRatings(mdb);const addRating=(source,value)=>{const n=Number(value);if(Number.isFinite(n)&&n>0&&!ratings.some(x=>x.source===source))ratings.push({source,value:n})};
  addRating('IMDb',merged.imdbRating||merged.imdb_rating);addRating('TMDB',tmdb?.vote_average||merged.tmdbRating||merged.tmdb_rating||merged.rating);
  const certification=(()=>{const rows=safeType==='movie'?tmdb?.release_dates?.results:tmdb?.content_ratings?.results;const gb=(rows||[]).find(x=>x.iso_3166_1==='GB')||(rows||[]).find(x=>x.iso_3166_1==='US');if(!gb)return '';if(safeType==='movie')return String(gb.release_dates?.find(x=>x.certification)?.certification||'');return String(gb.rating||'')})();

...
async function start(id,target){
 ...
 if(!copyAudio)throw new Error('This source needs audio transcoding, so AIOPlay skipped it.');
 ...
 const deadline=Date.now()+12000;while(Date.now()<deadline){if(fs.existsSync(playlist)&&fs.statSync(playlist).size>20)return{token,mode:'remux'};if(proc.exitCode!==null){stop(id);throw new Error('Safari-compatible HLS could not be created from this source.');}await new Promise(r=>setTimeout(r,120));}

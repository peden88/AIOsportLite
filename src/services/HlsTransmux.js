...
async function start(id,target){
 ...
 if(video&&video!=='H264'&&video!=='HEVC')throw new Error('This source needs video transcoding, so AIOPlay skipped it.');
 const audio=String(m.audio||'').toUpperCase(),copyAudio=!audio||/(?:AAC|DD\+|DD)(?:\s|\/|$)/.test(audio);
 ...
 args.push('-i',String(target.url),'-map','0:v:0','-map','0:a:0?','-c:v','copy');
 if(copyAudio)args.push('-c:a','copy');else args.push('-c:a','aac','-b:a','192k','-ac','2');
 ...
 const deadline=Date.now()+12000;while(Date.now()<deadline){if(fs.existsSync(playlist)&&fs.statSync(playlist).size>20)return{token,mode:copyAudio?'remux':'audio-transcode'};if(proc.exitCode!==null){stop(id);throw new Error('Safari-compatible HLS could not be created from this source.');}await new Promise(r=>setTimeout(r,120));}
 stop(id);throw new Error('Timed out preparing Safari-compatible playback.');
}
